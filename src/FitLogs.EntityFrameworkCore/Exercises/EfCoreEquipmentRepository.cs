...
public class EfCoreEquipmentRepository
    : EfCoreRepository<FitLogsDbContext, Equipment, Guid>,
      IEquipmentRepository
{
    public EfCoreEquipmentRepository(
        IDbContextProvider<FitLogsDbContext> dbContextProvider)
        : base(dbContextProvider)
    {
    }

    public async Task<Equipment?> FindByCodeAsync(string code)
    {
        ...
        int skipCount = 0)
    {
        ...
        query = query
            .OrderBy(string.IsNullOrWhiteSpace(sorting)
                ? $"{nameof(Equipment.DisplayOrder)} asc, {nameof(Equipment.Name)} asc"
                : sorting)
            .Skip(skipCount)
            .Take(maxResultCount);
    ...
    private static IQueryable<Equipment> ApplyFilter(
